...
class SeriesDetailsFragment(private val seriesId: String, private val parentView: View): DialogFragment() {

    private lateinit var binding: FragmentDetailsSeriesBinding
    private lateinit var detailsViewModel: SeriesDetailsViewModel
    ...
    private fun initializeViewModel() {
        detailsViewModel = ViewModelProvider(requireActivity().viewModelStore, SeriesDetailsViewModelFactory())
            .get(SeriesDetailsViewModel::class.java)
    }

    private fun observeViewModelChanges() {
        detailsViewModel.getDetailedItem(seriesId).observe(viewLifecycleOwner, Observer { firebaseSeries ->
            if (firebaseSeries != null)
                populateViewWithData(firebaseSeries)
        })
        ...
        detailsViewModel.currentItem.observe(viewLifecycleOwner, Observer {
            if (it?.id != seriesId) hideDetailsContent()
    ...
    private fun observeSectionChanges() {
        detailsViewModel.isSeriesInToWatch(seriesId).observe(viewLifecycleOwner, Observer {
            if (it == true) {
        ...
        detailsViewModel.isSeriesInWatched(seriesId).observe(viewLifecycleOwner, Observer {
            if (it == true) {
        ...
        detailsViewModel.isSeriesInFavourite(seriesId).observe(viewLifecycleOwner, Observer {
            if (it == true) {
    ...
    private fun openEditFragment() {
        val fragment = SeriesEditionFragment(parentView, true, detailsViewModel)
    ...
    private fun populateViewWithData(series: DetailedItem) {
        setPosterBasedOnUrl(series, binding.poster)
        binding.title.text = series.title
        binding.typeIcon.setImageResource(R.drawable.series_24)
        binding.year.text = series.year
        binding.seasons.text = series.totalSeasons
        binding.genre.text = series.genre
        binding.writer.text = series.writer
        binding.plot.text = series.plot
        binding.fabEdit.isVisible = series.source == ItemSource.FIREBASE
    }

    private fun setPosterBasedOnUrl(item: DetailedItem, view: ImageView) {
        if (item.posterUrl != "N/A" && item.posterUrl.isNotBlank())
