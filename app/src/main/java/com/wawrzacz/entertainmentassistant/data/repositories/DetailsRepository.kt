...
object DetailsRepository {
    ...
    private val gamesFirebaseRepository = GamesFirebaseRepository
    ...
    fun isGameInSection(id: String?, section: PlayableSection): LiveData<Boolean> {
        return gamesFirebaseRepository.getGameSectionValue(id, section)
    }
    ...
    fun getDetailedGame(id: String): LiveData<DetailedItemResponse> {
        return MediatorLiveData<DetailedItemResponse>().apply {
            var firebaseItemResult = DetailedItemResponse()
            var apiItemResult = DetailedItemResponse()
            ...
            addSource(gamesFirebaseRepository.getSingleItem(id)) {
                firebaseItemResult = it ?: DetailedItemResponse(null, ResponseStatus.NO_RESULT)
                checkResults()
            }
    ...
    fun toggleGameSection(section: PlayableSection, item: DetailedItem) {
